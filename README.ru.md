...
**Единая долговечная рабочая среда для Claude Code и Codex CLI: общая память, общие скиллы, ограниченная контрактом оркестрация и кросс‑ревью разными моделями.** Она превращает разговоры, PDF и другие документы, планы и решения в структурированную Obsidian‑вики, а затем передаёт эти знания следующим агентским сессиям вместо очередного старта с нуля.
...
| **Docling + EasyOCR** | Ставятся по умолчанию; для text-only работы опциональны | Локальная нормализация PDF, Office, EPUB и сканов до синтеза моделью, с явным русским и английским OCR. Это не hosted-сервис обработки документов. |
...
Передайте любому из двух агентов локальный путь к файлу. В обычной работе
конвертировать документ вручную не нужно:

```text
ingest ~/Downloads/architecture-report.pdf
```

Claude направит такой запрос в `/wiki-ingest`. Для Codex явный эквивалент —
`$llm-obsidian:wiki-ingest ~/Downloads/architecture-report.pdf`. Результат — не
PDF, вставленный в чат, а структурированные и перелинкованные страницы вики с
hash источника, provenance сессии и стабильными Obsidian-ссылками.

```text
локальный read-only файл
  -> проверка формата, размера и числа страниц
  -> быстрый stdlib-путь ИЛИ изолированный Docling + EasyOCR
  -> кешированные Markdown, таблицы и связанные изображения
  -> quality gate
  -> синтез через wiki-ingest
  -> одна транзакционная запись в вольт
```

| Вход | Путь обработки |
|---|---|
| Markdown, text, JSON, YAML, CSV | Python standard library; Docling не запускается |
| Локальный HTML | Networkless-очистка средствами standard library |
| PDF, DOCX, PPTX, XLSX, ODT/ODS/ODP, EPUB | Pinned локальный standard pipeline Docling |
| Скан документа | Docling + EasyOCR с явно заданными языками `ru,en` |
| Фотография, диаграмма или доска | Native vision-путь: основное содержание здесь визуальное, а не документный текст |

Docling работает до LLM, а не внутри неё. Профиль с точным разбором таблиц и
русско-английским OCR создаёт детерминированный Markdown-артефакт;
content-addressed кеш не повторяет конвертацию и OCR неизменившегося файла.
Оригинал остаётся read-only. Во время ingest отключены remote services, внешние
плагины Docling, удалённые изображения HTML и runtime-загрузка моделей.

Обычный bootstrap устанавливает изолированный runtime и заранее скачанные
модели. Для диагностики и ремонта достаточно этих команд:

```bash
bash bin/setup-clean-machine.sh
python3 scripts/document-normalize.py check --json
python3 scripts/install-docling.py install            # починить только Docling
python3 scripts/document-normalize.py normalize \
  "$HOME/Downloads/architecture-report.pdf" --json    # посмотреть сырой результат
```

Если Docling отсутствует, text-like файлы продолжают работать, а бинарные
источники возвращают понятный `needs_user_action`. Низкое качество распознавания
останавливает процесс до записи в вики. PDF никогда не отправляется модели
напрямую молча: model-native чтение бинарника — degraded fallback, который
требует отдельного подтверждения для конкретного источника. Лимиты по умолчанию:
250 МиБ, 2 000 страниц и 20 минут, без молчаливого обрезания. Кеш, статусы,
лимиты и ремонт подробно описаны в
...
После установки или обновления плагина начните новую сессию Claude Code или
Codex либо явно перезагрузите skills, если runtime предоставляет такую команду.
Уже открытые сессии сохраняют исходный реестр скиллов.
...
| Согласование | `/clarify` в Claude, `$llm-obsidian:clarify` в Codex (по одному вопросу о требованиях и дизайне до плана или реализации) |
...
Роутер на `UserPromptSubmit` подсказывает подходящий скилл по regex-правилам из `.claude/skill-rules.json` (мягкие подсказки, никогда не mandatory), включая явные запросы `clarify before code` и `grill me`; `session-nudge` поднимает просроченный maintenance (возраст линта, назревший fold, устаревшие бэкапы, совет skill-of-the-day).
