...
even narrower per-stage `CODEX_HOME` generated by the harness research workflow;
the thin `research-isolation.py` CLI only delegates lifecycle requests. Fetch
and synthesis do not inherit these profiles, MCP servers, plugins, or apps.
