...
| Deep review | Standalone, with no model/runtime override, this compiles independent `anthropic-holistic` and `openai-holistic` lanes on the configured deep routes. Inside a finalization lineage, cycles 1–3 compile only `finalization-primary` (Fable High). The exact third material failure freezes one packet for a single read-only `finalization-independent` review (Sol X-High); cycles 4–5 add that route only after an accepted structural-pivot receipt. Availability evidence does not authorize cycles 4–5. An explicit registered runtime/model override keeps both provider-prefixed intent and engineering lanes on that one model. |
