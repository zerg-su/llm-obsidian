---
type: meta
title: "Entities Index"
created: 2026-04-07
updated: 2026-07-11
tags:
  - meta
  - index
  - entity
status: evergreen
related:
  - "[[index]]"
  - "[[Andrej Karpathy]]"
  - "[[LLM Wiki Pattern]]"
sessions:
  - public-template-v2
---

# Сущности

Navigation: [[index]] | [[concepts/_index|Concepts]] | [[sources/_index|Sources]]

Сущности — люди, организации, продукты, инструменты.

---

## Люди

- [[Andrej Karpathy]] — AI researcher, автор LLM Wiki pattern

---

## Организации

*(добавляются по мере появления)*

---

## Продукты и инструменты

*(добавляются по мере исследования)*

---

*Новые сущности добавляются здесь по мере их идентификации в ингестах.*

<!-- AUTO-INDEX START -->
_1 pages, updated 2026-07-11_

- [[Andrej Karpathy]] — mature, 2026-07-11
<!-- AUTO-INDEX END -->
