...
Contract amendment authority: [[LLM Obsidian 2.6.4 — amendment plan-review outcome]]. Он добавляет E12 и подтверждает literal pointer-only disposition E8.
...
- deterministic compiler извлекает из одного плана четыре отдельные immutable artifacts: Outcome Contract, design, capability dispositions и success-evidence map; каждый получает независимый digest, даже когда source path один;
...
- **consumes:** Slices 1–4.
- **produces:** before/between/after-send races, both generic/submit orders, deadline expiry, accepted-after-timeout, stale/terminal/ownership/concurrency cases; design-only subject rebind accepted, Outcome/dispositions/evidence-map mutations rejected.
...
- **focused verification:** oracle and callback suites; E2, E4, E5, E11, E12.
...
- **minimal green:** compiler emits four independently hashed artifacts; only typed design delta may rebind in retained lanes; protected artifact changes require amendment/fresh boundary; common single-parent plan commit auto base, otherwise explicit exact base; no symbolic resolver/broad Bash.
- **refactor seam:** plan boundary compiler is deep module; current facade delegates once.
- **focused verification:** assert zero `RuntimeSessionManager.start` on invalid combinations; unchanged protected artifacts plus exact design delta continue both retained lanes with zero new sessions; Outcome/disposition/evidence-map mutations fail closed; exact suite deltas are 12 new Python cases, +2 inspect cases and +4 DCG assertions; E9, E11, E12.
