...
3. Run Stability Gate S: exact D-264-73 and stale-exiting repro, generated event/retry interleavings, max one provider-facing effect, deadline zero authority, no helper/reviewer/eventual-file leaks, complexity/writable-authority reduction, full/coverage/acceptance/clean HEAD. If not green within bounded budget, finish lifecycle-only partial outcome and leave Split disabled.
   - D-265-VRF-01: replace HEAD-only verification identity with an immutable bounded `VerificationAttempt` (`parent + profile + exact HEAD + attempt_index`). One coordinator-authorized same-HEAD retry may create attempt 1 only after a typed mechanism-flake decision; it preserves attempt 0 evidence, starts no model/provider, never requires an empty commit, and a second same-HEAD retry stops with typed attention. Changed-HEAD `fix-and-resubmit` remains a distinct path.
