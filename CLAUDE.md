...
**Skills:** `/wiki`, `/wiki-ingest`, `/wiki-query`, `/wiki-lint`, `/wiki-fold`, `/save`, `/save-plan`, `/close`, `/autoresearch`, `/canvas`, `/daily`, `/journal`, `/backlog`, `/find-session`, `/draft`, `/distill-runbook`, `/learn`, `/defuddle`, `/dispatch`, `/review-dispatch`, `/review-send`, `/reap`, `/reap-send` (+ reference: obsidian-markdown, obsidian-bases)
...
Ядро: `ingest <путь|URL>` (источник → страницы в wiki/), `что ты знаешь про X?` (поиск с цитатами), `/save` (зафиксировать разговор), `/journal` (план на дату), `/backlog add` (не забыть), `lint the wiki`, `update hot cache`. Параллельные задачи: `/dispatch` + `/review-dispatch` + `/reap-send` (требует cmux). Полный каталог скиллов: [[getting-started]].
