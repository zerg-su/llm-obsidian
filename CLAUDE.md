...
- **Claude Code hooks** (`hooks/hooks.json`, `.claude/hooks/`) — Claude-specific. Codex не исполняет этот hook layer; если hook-конфиг случайно попадёт в Codex-среду, скрипты выходят по `CODEX_THREAD_ID` guard.
