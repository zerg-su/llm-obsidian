...
- **`PostToolUse[Bash|exec_command|shell|unified_exec]`**: строгая нормализация и санитизированный лог команд в `.vault-meta/command-log.jsonl`; provenance и execution session хранятся раздельно (сырьё для /distill-runbook).
- **`PostToolUse[ExitPlanMode]`**: `plan-capture.sh` — каждый approved план → `wiki/plans/`.
- **`Stop`**: shell-wrapper → `stop-hook.py`: transaction recovery → reindex → section sparse ensure → fingerprinted deferred-dense marker/worker → opt-in memory backup → strict validate → scoped commit (legacy BM25 индекс пока тоже self-heal'ится для compatibility). Stop не ждёт embeddings; dense worker сериализован отдельным `fcntl` lock, а hybrid до его завершения деградирует в sparse. Validation failure или небезопасный memory snapshot блокирует commit — и такой заблокированный turn-end теперь виден в сессии как `systemMessage`, а не только в `.vault-meta/stop-hook-last.log`. Required budget по умолчанию 60s; override: `LLM_OBSIDIAN_STOP_REQUIRED_TIMEOUT_SEC` (1–600s). Ожидание pipeline-lock по умолчанию 2s; override: `LLM_OBSIDIAN_STOP_LOCK_TIMEOUT_SEC` (1–600s) — `/close` поднимает его, чтобы синхронный turn-end не сдался на busy-локе. Memory backup не угадывает пути: только `CLAUDE_MEMORY_DIR` или локальный `.vault-meta/memory-backup.json` из `config/memory-backup.example.json`.
...
D-265-EPH-01 разрешает `claude -p` только внутри registered code-owned ephemeral adapter для bounded review/schema work после subscription preflight, fixed argv compilation, minimal context, schema validation, bounded capability checks и durable receipts. Arbitrary direct print-mode review/dispatch остаётся запрещён; continuable work и interactive capabilities по-прежнему требуют видимую Claude Code-сессию в cmux split. Ephemeral failure не запускает hidden interactive, paid credits или provider fallback. Task executor запускается с выбранным рабочим permission mode; read-only reviewer — с locked-down `dontAsk` и точным tool allow-list. После утверждённого unattended-плана routine review/reap не спрашивают повторных подтверждений; blocking/scope drift эскалируются. Observer-only watchdog уведомляет после 15/20 минут без видимого прогресса, но отправляет input и не останавливает агента. Завершённые фоновые процессы закрывают только собственный armed cmux surface; coordinator/workspace не закрываются.
