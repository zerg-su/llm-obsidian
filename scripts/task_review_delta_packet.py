...
def build_delta_packet(
    ...
    if not delta or len(delta) > MAX_DELTA_TOTAL_BYTES:
        raise DeltaPacketError(
            "review delta must be non-empty and at most 131072 bytes"
        )
    try:
        delta.decode("utf-8")
...
def validate_materialized_delta_packet(
    ...
    *,
    ...
    part_names = sorted(name for name in fix_rows if name != manifest_name)
    if manifest_name not in fix_rows or part_names not in (
    ...
    def materialized(name: str) -> bytes:
    ...
    return validate_delta_packet(
        materialized(manifest_name),
        {name: materialized(name) for name in part_names},
