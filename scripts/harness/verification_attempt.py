...
def build_verification_escalation(
    ...
    if (
        ...
        raise VerificationAttemptError(
            "verification escalation identity is invalid"
        )
    return {
        "schema_version": 1,
...
def resolve_verification_escalation(
    ...
    *,
    ...
    if not isinstance(escalation, dict):
        raise VerificationAttemptError("verification escalation is invalid")
    expected = {
        "schema_version",
        "kind",
        "category",
    ...
    if (
        ...
        or not IDENTIFIER.fullmatch(
            str(escalation.get("verification_operation_id") or "")
        )
        or not GIT_OID.fullmatch(str(escalation.get("exact_head_sha") or ""))
        or not SHA256.fullmatch(
            str(escalation.get("failed_attempt_sha256") or "")
        )
    ...
    return {
        **escalation,
        "decision": (
            "authorize-attempt-1"
            if action == "authorize-one-same-head-retry"
