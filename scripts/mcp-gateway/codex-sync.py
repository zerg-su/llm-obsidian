...
DEFAULT_PROFILE = "llm-obsidian-mcp"
...
def profile_paths(repo_root: Path) -> OrderedDict[str, Path]:
    ...
    for path in sorted(profiles_dir.glob("*.json")):
        out[f"llm-obsidian-{path.stem}"] = path
    return out
...
def profile_text(repo_root: Path, profile_path: Path | None) -> str:
    ...
    return (
        "# llm-obsidian Codex MCP profile.\n"
...
def desired_files(repo_root: Path, codex_home: Path) -> dict[Path, str]:
    ...
    files = {
        ...
        codex_home / f"{DEFAULT_PROFILE}.config.toml": profile_text(repo_root, None),
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Sync Codex MCP TOML config from llm-obsidian MCP JSON files.")
    ...
    backup_dir = codex_home / "backups" / f"llm-obsidian-codex-sync-{stamp}"
