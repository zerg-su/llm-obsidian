...
with tempfile.TemporaryDirectory(prefix="custom-runtime.") as raw:
    ...
    # Keep both initial-delivery races under the two registered profiles.  The
    # change profile publishes the callback before the worker observes it; the
    # fix profile lets the already-launched provider consume its initial request
    # while the callback is still absent.
    if PROFILE == "change":
    ...
    if (
        ...
        or notification_cmux.initial_notification_attempts != 0
        or notification_cmux.successor_notification_attempts != 1
        ...
        raise AssertionError(
            (
                ...
                notification_cmux.initial_notification_attempts,
                notification_cmux.successor_notification_attempts,
