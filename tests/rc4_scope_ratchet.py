...
The third accepted 2.7.2 Sol review round then closed the ancestor variant of
the same escape: ``Path.is_symlink()`` checks only the final entry, so the
no-follow guard now validates every component from the trusted store root
through ``owners/<owner>/runtime/<operation>/provider-events/generation-1``
...
The 2.7.5 durable exact consumer admission closes the frozen
F274.POST_CHECK_LAUNCH_RACE class: the durable controller link is admitted
by an exact receipt/HEAD re-observation at its own write boundary (15 lines
in the verification owner), the review drive publishes the exact durable
receipt/HEAD pair as the launch admission instead of a carried boolean (65
lines in the review bridge), and the exact-HEAD flow verifies that admission
against the actual review context before reservation or provider effect (69
...
identity-bound write-ahead stages plus thread/process linearization. The exact
 ...
 exactly 135 lines to existing review owners: checkpoint state is refreshed
...
#: Maximum tracked Python files under ``scripts/`` for the 2.8.6 candidate.
...
#: Maximum total lines across those files for the 2.8.6 lifecycle candidate.
