...
A `UserPromptSubmit` router suggests the right skill from regex rules in `.claude/skill-rules.json` (soft hints, never mandatory), including explicit `clarify before code` / `grill me` requests; `session-nudge` surfaces overdue maintenance (lint age, fold due, stale backups, a skill-of-the-day tip).
