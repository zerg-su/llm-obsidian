...
- Доставка в cmux composer снова разделяет владельцев ввода и отправки:
  `cmux send` записывает prompt, Harness подтверждает состояние текущего
  редактора, а ровно один `Enter` отправляет его. Устаревшие ordered-paste
  capability и путь с неявной отправкой удалены.
...
- Onboarding окружения для auto mode в Claude теперь считается native dialog
  провайдера, а не task authority. Harness закрывает точный onboarding и уже
  открытый setup wizard через `Esc`, не выбирая окружение, не меняя постоянные
  предпочтения пользователя и не запрашивая task approval.
- Retained-session continuation теперь проверяет текущий экран провайдера до
  reservation или paste task input. Любой известный или неизвестный native
  dialog блокирует paste, поэтому typed-переход custom, fix, review или summary
  больше не может попасть в provider UI.

### Проверка

- Точные переходы onboarding и wizard проходят 50 stateful-повторов. Матрица
  registered transition transport остаётся зелёной для девяти production-
  коридоров по 50 повторов, полный Harness gate проходит.
- Контур scripts сдвинут на точные 35 production-строк исправления: 297 файлов
  / 114 118 строк без спекулятивного запаса.

## [2.8.8] — 2026-08-20

### Исправлено

- Переходы между agent editor теперь используют capability-advertised ordered
  paste cmux с точной идентичностью surface. Executor, review, custom, fix,
  verification, summary, liveness, callback и coordinator escalation работают
  через один транспорт, сохраняя раздельные evidence для paste и submit.
- Continuation delivery пережидает краткий неизменившийся idle frame вместо
  немедленного ошибочного вывода, что paste не состоялся.
...
- Живой прототип Claude Sonnet Low воспроизвёл старую потерю input, после чего
  выполнил десять последовательных ordered-paste доставок. Production adapter
...
- Доставка current-review в Codex теперь ждёт окончания промежуточного выбора
  доверия без footer и только затем считает редактор готовым. Начальный prompt
  ревью больше не остаётся неотправленным после закрытия trust dialog.
- Regression покрывает и видимый переход trust dialog, и dialog, который
  появляется после первого idle-кадра. Live ratchet scripts сдвинут на точную
  чистую production-стоимость в двенадцать строк — до 296 файлов / 113 215
  строк без спекулятивного запаса.

## [2.8.4] — 2026-08-19

### Исправлено

- Первый шаг custom pipeline, уже принадлежащий запущенному provider'у, больше
  не доставляется повторно через recovery retained-сессии, пока callback ещё
  не опубликован. Successor steps сохраняют существующее одноразовое
  уведомление, а уже опубликованный callback по-прежнему приоритетнее transport.
- Regression matrix теперь покрывает обе гонки первого custom step: callback,
  опубликованный до наблюдения, и отсутствие callback, пока запросом уже
  владеет запущенный provider. Полный repository gate и Simple review на Sol
  проходят на точном кандидате.
...
- Явно авторизованная команда `cancel-stale` теперь без replay закрывает только
  просроченные и узко классифицированные pending effects, отказывает при живых
  или неизвестных ресурсах и атомарно отменяет точное поддерево. Контур active
  authority зафиксирован на 28 файлах / 16 387 LOC, а live ratchet scripts —
  на 296 файлах / 113 203 строках; оба значения без спекулятивного запаса.
- Upgrade preflight теперь учитывает durable `archive_status` завершённого
  review и не принимает сохранённый close-armed transport marker за живую
  работу.
...
- Harness liveness теперь читает строгий semantic status cmux для точного
  workspace и обеих сред: Claude (`claude_code`) и Codex (`codex`). Динамическая
  перерисовка счётчиков и строки статуса больше не сбрасывает idle-таймер, когда
  provider сообщает `Idle` или `Needs input`.
- Уведомления retained-сессий в custom и engineering/fix pipeline ждут, пока
...
- Продолжение lifecycle теперь принимает точный вложенный review callback через
  ...
  scripts сдвинут к измеренному кандидату 294 файла / 111 456 строк с нулевым
  запасом. По явному решению координатора отдельно измеренный дрейф в 26 строк
  предшествует этому исправлению и принят точно, без спекулятивного запаса.
- Авторизованный координатором примитив
  `task-review-runner.py authorized-continuation` связывает одно точное принятое
  решение escalation и изменённый Outcome Contract с чистым successor HEAD,
  одним неизменяемым receipt полной верификации и одним свежим продолжением
  Deep review, не переписывая task metadata и предыдущие records. Единственный
  production owner и зарегистрированная точка входа сдвигают live ratchet на
...
- Findings Deep review этого handoff применены в том же owner: attention packet
  больше не пишется writer'ом, ужимающим каталог, поэтому публикация attention
  packet не меняет caller-owned корень checkout с 0755 на 0700 (binding на
  success-path через `_write_once` — тоже checkout-root артефакт и всё ещё
  меняет; сужение этого parent-owned; сам выбор writer'а заменён staged-публикацией
  через `os.link` ниже); size guard
  измеряет ровно те байты, которые пишет writer, поэтому граница файла у
  стандартного consumer'а совпадает с проверяемой здесь; attention packet,
  который эта identity не выводила, отклоняется, а не заменяется, и
  опубликованный packet снимается только решением координатора; выбор
  терминального `failed` и путь опубликованного packet названы там, где
  поднимается отказ. Точно измеренные +18 строк до 295 файлов / 112 264 строк
  без запаса.
- Коридоры identity-bound resubmission публикуют
  `.task-verification-response.json` атомарно. Фикстура писала этот общий
  control-артефакт неатомарным writer'ом, пока worker его параллельно опрашивает,
  поэтому reader мог попасть в окно усечения до нуля байт; настоящий consumer
  отклоняет пустое чтение как невалидный response, и dispatch фиксирует
  `callback-invalid` без второй попытки верификации. Измерено напрямую:
  неатомарный writer показал невалидное состояние в 10 212 из 16 725
  параллельных наблюдений, атомарный — в 0 из 44 761. Оба коридора используют
  существующий publisher `write_json_atomic`, sleep 0.12s, который сужал, но не
  закрывал окно, удалён, а детерминированная регрессия фиксирует, что атомарная
  публикация наблюдается только полной, а нулевая по-прежнему детектируется как
  невалидная. Production-код не менялся.
- Второй раунд review этого handoff применён в том же owner: детерминированная
  регрессия привязана к собственной публикации коридора через идиому
  `publication_barrier`, уже принадлежащую этому файлу, поэтому откат publisher'а
  теперь валит проверку, а не деградирует в перемежающийся end-to-end flake;
  observer запускается через `start_checked_thread`, поэтому сбой пробы — это
  evidence главного потока; классификатор состояния response сведён к трём
  состояниям, которые реально проверяются; attention packet публикуется
  create-exclusive (`O_WRONLY|O_CREAT|O_EXCL`), поэтому решение об ownership
  «отказать, а не заменить» и запись стали одним шагом вместо check-then-write.
  Точно измеренные +37 строк до 295 файлов / 112 301 строки без запаса.
- Третий раунд review сохраняет одновременно create-exclusive ownership **и**
  атомарную полную публикацию, которой предыдущий раунд пожертвовал: packet
  сначала пишется в staged-файл 0600 с flush и fsync, затем публикуется через
  `os.link`, поэтому решение об ownership и запись остаются одним шагом, а
  опрашиваемый путь появляется только полностью сформированным. Прерывание или
  падение теперь оставляет лишь игнорируемый staged-файл вместо нулевого packet,
  который навсегда заклинивал failed-receipt handoff: прежняя схема
  `O_EXCL`-затем-запись создавала путь раньше содержимого, а `BaseException`
  обходил её очистку. Уже опубликованный packet читается ограниченно, без
  перехода по symlink и только как обычный файл, поэтому symlink или FIFO на этом
  пути отклоняется, а не блокирует публикацию. Все ветки отказа покрыты
  зарегистрированным owner'ом: changed, unreadable, too large, invalid и очистка
  после прерывания. Точно измеренные +44 строки до 295 файлов / 112 345 строк без
  запаса.
- Verification resubmission теперь согласует changed-HEAD response, который
  появляется после того, как worker уже привязал failed HEAD для текущего
  прохода. В устаревшем проходе новая authority не принимается: распознаётся
  только точный identity-bound response для текущего чистого successor, после
  чего следующий обычный reconcile потребляет его через существующий
  changed-HEAD путь. Детерминированная межпоточная регрессия заменяет flake
  полного набора. Существующий owner верификации растёт ровно на 37 строк;
  active-authority контур закреплён на 15 839 LOC, а live ratchet scripts — на
  295 файлах / 112 382 строках, в обоих случаях без запаса.

## [2.8.0] - 2026-08-17

Architecture Workflow v1: архитектурный замысел становится принятым проектным
знанием, а затем исполняемым Work Item DAG. Подробности:
[docs/releases/v2.8.0.md](docs/releases/v2.8.0.md).

### Добавлено

- Carrier `architecture`: владеет Design Frontier и знанием о системе целиком
  (vision, architecture, bounded design, spec, contract) под общим
  artifact-контрактом, с одним явным handoff в carrier и возвратом артефакта в
  проектный контекст.
- Carrier `decompose`: собирает `<Project> Work Graph` и его Work Items из
  принятого знания через MAP, ACCEPT и авторизованный MATERIALIZE, доказывает
  двустороннюю трассируемость и выводит волны и Planning Frontier из
  `depends_on` плюс реальных ограничений параллелизма. Пути к файлам и TDD
  остаются ниже границы Work Item.
- `docs/skill-references/architecture-artifacts.md`: единый artifact-контракт,
  который оба carrier'а читают полностью — роли, размещение под
  `wiki/projects/<project>/`, идентичность «заголовок плюс адрес», ревизии,
  report-only freshness, жизненный цикл и инвариант durable consumption.
- `scripts/architecture_paths.py`: ограниченный валидатор путей — грамматика,
  ключ коллизий `NFC(title).casefold()` против всего link-namespace включая
  алиасы, отображение роли в папку, отказ на симлинке в любом компоненте от
  корня вольта до выбранной role-директории и равенство канонического
  назначения.
- `implementation-plan` принимает на вход Work Item: проверяет авторитетность
  upstream по классам, тотальность и свежесть пинов, согласованность
  recovery/projection до любого планирования файлов и TDD, иначе выдаёт
  канонический Upstream Gap.
- `scripts/architecture_workflow_pressure.py` и
  `scripts/architecture_workflow_audit.py`: собственная машинерия поведенческих
  доказательств релиза с провенансом свежей сессии на кейс и fail-closed
  санитизацией полной записи.

### Исправлено

- Замороженный RC4-знаменатель прибивает точные байты `AGENTS.md` и
  `CLAUDE.md`  из-за чего каталог скиллов внутри `CLAUDE.md` тоже оказался
  заморожен: любой релиз с новым скиллом ломал историческое evidence.
  `normalized_governing_source` теперь канонизирует только строку каталога:
  исторические пины и дайджесты промптов остаются байт-идентичными  а
  руководство — актуальным; регрессия доказывает  что любой другой дрейф
  `CLAUDE.md` по-прежнему падает закрыто. Ratchet: 290 файлов / 109 677 строк.

- Замороженный RC4-знаменатель engineering discipline больше не ломается,
  когда релиз поставляет новый скилл. Он прибивает точные байты `AGENTS.md` и
  `CLAUDE.md`, потому что записанные промпты v2.6.6 обязаны оставаться
  воспроизводимыми, но каталог скиллов внутри `CLAUDE.md` — это индекс, а не
  один из проверяемых принципов. `normalized_governing_source` теперь
  канонизирует только строку каталога: исторические пины и дайджесты промптов
  остаются байт-идентичными, а руководство — актуальным. Регрессия доказывает,
  что поставка нового скилла принимается, а любой другой дрейф `CLAUDE.md`
  по-прежнему падает закрыто. Историческое evidence не изменено. Ratchet объёма scripts сдвигается к измеренному кандидату 293 файла / 110 718 строк.

- Ограниченная fix-delta ревью-резолюции теперь считается одной
  зарегистрированной git-командой (`review_resolution.fix_delta_command`),
  исключающей закоммиченные байты `docs/acceptance/evidence/`: acceptance
  evidence больше не может вытеснить маленький продуктовый фикс из
  транспортного капа 1 MiB; кап по-прежнему действует для продуктовых путей,
  а evidence-only резолюция остаётся пустой и отклоняется. Оба вычисления в
  resolution bundle и независимая digest-проверка review program привязаны к
  одной команде. Потолок ratchet для этого релиза указан один раз ниже.

### Изменено

- Architecture Workflow v1 сдвигает интегрированный ratchet 2.7.8 на свою
  точно измеренную production-цену к 293 файлам / 110 718 строкам с нулевым
  запасом: 252-строчный `scripts/architecture_paths.py`, 276-строчный
  `scripts/architecture_workflow_audit.py` и 780-строчный
  `scripts/architecture_workflow_pressure.py` принадлежат релизу, а общий
  engineering-eval runner остаётся побайтово идентичным реконсилированной базе
  main после отдельно владеемой 29-строчной проекции каталога.
